import { miruSourceId } from "./config.js";
import { getMeta, getRuntime, getInstalledSources } from "./repo.js";

export function buildStreamPayload(streamUrl, pageUrl, extraHeaders = {}) {
  const type =
    /\.m3u8(\?|$)/i.test(streamUrl) || /mpegurl/i.test(streamUrl) ? "hls" : "file";
  return {
    url: streamUrl,
    sourceUrl: streamUrl,
    type,
...
export async function resolveMiruPlay(packageName, watchUrl, pageUrl = "") {
  ...
  if (type === "torrent") {
    throw new Error("该 Miru 扩展返回 BT 资源，暂不支持，请换源");
  }
  ...
  if (type === "hls" || type === "mp4" || type === "file") {
    payload.type =
      type === "mp4" || type === "file" ? "file" : type === "hls" ? "hls" : payload.type;
  }
